...
export default function Listings() {
  ...
  const areas = useMemo(
    () =>
      Array.from(
        new Set(allListings.map((l) => pickLocale(l.area, "en")))
      ).sort(),
    []
  );
  ...
  return (
    <Wrap>
      ...
      <Controls>
        ...
        >
          <option value="relevant">
            {t("listings.sort.relevant", "Sort: Relevant")}
          </option>
          <option value="low">
            {t("listings.sort.low", "Price: Low to High")}
          </option>
          <option value="high">
            {t("listings.sort.high", "Price: High to Low")}
          </option>
        </Select>
      </Controls>
      ...
      {filtered.length === 0 && (
        <Empty>
          {t("listings.empty", "No results. Try different filters.")}
        </Empty>
      )}
...
`;
const Media = styled.div<{ img: string }>`
  height: 150px;
